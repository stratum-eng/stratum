...
describe("captureDeletionTarget", () => {
  ...
  it("tolerates a corrupt workspace entry: still captures its name, skips its fork", async () => {
    const { db } = makeRecordingD1((sql) => {
      if (sql.includes("FROM changes")) return [];
      if (sql.includes("FROM webhooks")) return [];
      return [];
    });
    const kvStub = makeKvStub();
    kvStub.store.set(
      "workspace:proj_1:ws-good",
      JSON.stringify({
        remote: "https://acct.artifacts.cloudflare.net/git/@alice/ws-good-fork.git",
      }),
    );
    kvStub.store.set("workspace:proj_1:ws-bad", "{not valid json");
    const project = makeProject();
    kvStub.store.set("project:@alice:api", JSON.stringify(project));
    ...
    expect(result.success).toBe(true);
    if (!result.success) return;
    // Both workspace names are captured from the key listing alone...
    expect(result.data.workspaceNames.sort()).toEqual(["ws-bad", "ws-good"]);
    // ...but only the parseable entry contributes a resolvable fork repo.
    expect(result.data.forkRepoNames).toHaveLength(1);
    expect(mockLogger.error).toHaveBeenCalledWith(
      "Failed to parse workspace entry during deletion capture",
      expect.anything(),
      { key: "workspace:proj_1:ws-bad" },
    );
  });
});
...
describe("deleteProjectCascade", () => {
  ...
  it("reports a failing KV delete as a residual instead of throwing", async () => {
    const { db } = makeRecordingD1();
    // Wrap the stub KV so the policy key delete fails, but everything else
    // behaves normally — the cascade must not abort partway through.
    const flakyKv: KVNamespace = {
      ...kvStub.kv,
      delete: async (key: string) => {
        if (key === "policy:proj_1") throw new Error("KV down");
        return kvStub.kv.delete(key);
      },
    } as unknown as KVNamespace;

    const result = await deleteProjectCascade(
      makeEnv({ db, kv: flakyKv }),
      makeTarget(),
      mockLogger,
    );
    ...
    expect(result.success).toBe(true);
    if (!result.success) return;
    expect(result.data.residuals).toEqual(["kv:policy:proj_1"]);
    // Every other KV key still got cleaned up.
    expect(kvStub.store.has("project:@alice:api")).toBe(false);
    expect(kvStub.store.has("workspace:proj_1:ws-a")).toBe(false);
  });
});
...
describe("verifyProjectDeleted", () => {
  ...
  it("never calls Artifacts (documented: no cheap existence check is available)", async () => {
    const { db } = makeRecordingD1(() => [{ n: 0 }]);
    const artifactsStub = makeArtifactsStub();

    const result = await verifyProjectDeleted(
      makeEnv({ db, kv: makeKvStub().kv, artifacts: artifactsStub.artifacts }),
      makeTarget(),
      mockLogger,
    );

    expect(result.success).toBe(true);
    expect(artifactsStub.attempts).toEqual([]);
  });
});

...
describe("parseEditPlan", () => {
  ...
  it("rejects invalid JSON, empty plans, and unsafe paths", () => {
    ...
    expect(() => parseEditPlan('{"files": {}, "commitMessage": "m"}')).toThrow(/no file changes/);
    expect(() => parseEditPlan('{"files": {"../etc/passwd": "x"}, "commitMessage": "m"}')).toThrow(
      /unsafe path/,
    );
    expect(() => parseEditPlan('{"files": {"/abs/path": "x"}, "commitMessage": "m"}')).toThrow(
      /unsafe path/,
    );
    expect(() => parseEditPlan('{"files": {"a.ts": 42}, "commitMessage": "m"}')).toThrow(
      /non-string content/,
    );
  });
});
